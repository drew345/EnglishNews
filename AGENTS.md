...
English News is a companion for Korean-native speakers practicing English through the same news stories. Independent English text preparation is implemented in the existing development worktrees. It consumes shared facts or the next incoming run’s English sentence pairs, selects English vocabulary directly, and hands reviewed text to the audio/video workflow. The current implementation is tested; the first live vocabulary draft from 20260920_095703_18b0b84b awaits text review. See the latest SESSION_LOG.md entry for flagged choices and general morphology/phrase-selection issues. Listening review and main adoption remain pending. See development/README.md. Automatic daily triggering and upload are not enabled.
...
- Andrew now wants independent vocabulary selection from the actual English lesson, including useful phrases, with a soft target of 8–12 items per story. Use English frequency data; the proposed Korean-rank mapping is superseded. The implementation uses wordfreq 3.1.1 top-6,000 surface-token ranks with configurable cutoff 3,500, taking the more common of surface/lemma rank. Phrases receive independent assessment.
...
- Selection uses spaCy tokenization/lemmas/entities, common-word and manual gates, then contextual usefulness, familiar same-meaning loanwords and definitions. Include overrides bypass frequency only; exclude overrides block matching forms/lemmas. Names/grammar/entity/meaning checks still apply. Deduplicate within a story and allow up to three occurrences of a word/meaning across a run.
