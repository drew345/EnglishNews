...
Keep one managed YouTube tab in this task, reusing it across days and between Studio and public comments. An older published video does not require a new tab. Close only verified obsolete own published-video tabs; never another task's provider tab or an unknown/unfinished tab. Release the browser reservation after navigation/verification and before drafting comment text or waiting for follow-up requests. Text-only drafting does not reserve the browser. See the shared runbook for ownership checks.
