...
- English full review; separate YouTube channel with Korean titles/descriptions. Andrew confirmed Alloy, English 0.88 and Korean sentences 1.07 for the initial sample; vocabulary blocks and review use 0.88. The channel identity is recorded below; English audience copy is owned by english_news/audience.json.
...
- English explanation policy v2 uses one dictionary-style phrase, normally 4–8 simple words (shorter allowed), with a software maximum of 10 and narrow circularity checks. Meaningful related-word reuse is allowed. Routine selection includes this guidance in its existing call; explicit `python -m english_news.definitions --prepared-run ...` revisions preserve selected vocabulary and Korean glosses. Latest full written review: output/text-review/20260920_english_text_69f2def3b90e, superseding 26d4238eef86 for definition review. Andrew approved the vocabulary choices; shorter definitions await review.
- Selection uses spaCy tokenization/grammar/entities, dictionary-backed morphology, a controlled positive phrase reference, common-word and manual gates, then contextual usefulness, familiar same-meaning loanwords and definitions in the same request. Required context/learning-unit judgments may veto entries, never override exclusions or write rule files. easy-overrides.json is an optional separate overlay and starts empty. Include overrides bypass frequency only; easy-list, morphology and phrase gates remain enforced. Exclude overrides block matching forms/lemmas. Names/grammar/entity/meaning checks still apply. Deduplicate within a story and allow up to three occurrences of a word/meaning across a run.
...
- September 18/19 outputs supplied offline diagnostics. The first live selection used the September 20 incoming run; its draft is not yet approved. Preserve the distinction between tests, model output and human acceptance.
