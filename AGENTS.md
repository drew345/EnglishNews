...
English News is a companion for Korean-native speakers practicing English through the same news stories. Independent English text preparation is implemented in the existing development worktrees. It consumes shared facts or the next incoming run’s English sentence pairs, selects English vocabulary directly, and hands reviewed text to the audio/video workflow. The current implementation is offline-tested; next-run semantic/listening review and main adoption remain pending. See development/README.md. Automatic daily triggering and upload are not enabled.
...
- Real September 18/19 outputs were available for offline diagnostics on LG Gram 14. No newer incoming run was present during implementation; do not treat historical diagnostics as live semantic validation.
