...
Following review of the first live draft, Andrew approved stronger deterministic selection rules and a text-only rerun of the same current lesson. Selector v4 retains dictionary round-trip validation and the positive expression/pattern reference introduced in v3. Unknown/ambiguous inflections and unrecognized phrases are omitted and audited. The separate easy overlay now contains user-approved tourism; phrase rules exclude start over and numeric-maximum up to. Other senses remain eligible. No source frequency data was edited and no individual exception was needed for hopes. The cutoff remains 3,500.

The existing selection/definition call includes mandatory contextual and learning-unit vetoes, with no extra review call. Only eligible IDs are sent; model output cannot override software exclusions or modify permanent lists. This is conservative software eligibility plus contextual judgment, not a claim that software can decide all learner usefulness. English definitions now follow one-phrase dictionary guidance: normally 4–8 simple words, shorter when sufficient, with a software ceiling of 10 and narrow circularity checks. Preserve essential meaning and permit meaningful related-word reuse. An explicit text-only definition revision can preserve an approved selection while creating a separate review bundle; routine generation retains one selection/definition request. Detailed maintenance rules and commands are in development/README.md.

All 54 English tests pass. The current text draft is `output/text-review/20260920_english_text_69271a0759a5`, with 1/6/5 items and unchanged sentence pairs. The same model call now separately assesses learning value for Korean adults with substantial English vocabulary; false is enforced even when usefulness is high. The three confirmed easy items and nine additional items were omitted, so sparse story 1 is a review point. Andrew approved the short definitions and TTS format; revised vocabulary still awaits review. No manual selection edits, new media or production changes. Earlier execution counts and first-run observations below are historical checkpoints.
