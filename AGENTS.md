...
English News is a companion for Korean-native speakers practicing English through the same news stories. Independent English text preparation is implemented in the existing development worktrees. It consumes shared facts or the next incoming run’s English sentence pairs, selects English vocabulary directly, and hands reviewed text to the audio/video workflow. The current implementation is tested; the headline-updated lesson 20260920_english_text_8da79538915d (1/6/5 items) has been rendered at output/runs/20260920_english_all_3446bd4d60, with audio/video QA passed and readability approved. The subsequent continuous-scroll correction is tested but awaits the next authorized media build. See the latest SESSION_LOG.md entry for implemented software safeguards and remaining coverage limits. Listening review and main adoption remain pending. See development/README.md. Automatic daily triggering and upload are not enabled.
